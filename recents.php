...
class Recent_Comments extends WP_Widget {
    function __construct() {
        $widget_ops = array('classname' => 'my_widget_class', 'description' => __('Widget to display recent comments.', 'awp'));
    ...
    function widget( $args, $instance ) {
        ...
        if ( $comments ) {
            ...
            foreach ( (array) $comments as $comment) {
                ...
                $output .=  '<li class="recentcomments"><div class="post-comment">' .sprintf(_x('%1$s on %2$s  %3$s %4$s', 'awp'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . $posttitle . '</a></div>' ,'<div class="post-comm-avatar">'.$image .'</div>','<div class="comment-text">'.$com.'</div>') . '</li>';
            }
        }
        $output .= $after_widget;
        echo $output;
